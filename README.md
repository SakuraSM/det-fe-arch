...
DFA（Deterministic Frontend Architect）把前端工程最佳实践沉淀为 Agent Skill、参考规则、脚本和模板，用于约束 AI 生成的 React/Vue 3/TypeScript 代码。

## 目标

- 减少 LLM 生成前端代码时的命名混乱、职责耦合、魔术值和框架反模式。
- 对 React、Vue 3、A11y、Clean Code 提供可按需加载的规则集。
- 通过脚本执行 TypeScript、ESLint、行数和 A11y 后置检查。
...
| 维度 | 文件 | 重点 |
|---|---|---|
| 通用工程纪律 | `SKILL.md`、`references/clean-code.md` | 命名、魔术值、职责拆分、重构触发器、TypeScript 安全 |
...
`evals/test-cases.json` 提供 12 个覆盖 clean code、重构、A11y 和框架约束的测试 Prompt。`evals/assertions.json` 定义对应断言，可用于人工评审或自动化评测。
